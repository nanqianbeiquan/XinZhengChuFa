...
class html_guangdong(object):
    ...
    def get_filename_list_1(self):
        """
        返回广东省,filename为htm的列表
        1为非正常户
        """
        self.mysql_conn()
        sql_1 = "SELECT * FROM taxplayer_filename where province='广东省' AND title like '%非正常户%' \
    ...
    def get_filename_list_2(self):
        ...
        """
        self.mysql_conn()
        sql_2 = "SELECT * FROM taxplayer_filename where province='安徽省' AND title not like '%非正常户%' \
        AND filename LIKE '%htm%'"
    ...
    def get_filename_path_list_1(self):
        '''
        返回返回广东省,filename为htm的path 列表
        1为非正常户
        '''
        filename_path_list_1 = []
        fbrq_list_1 = []
        filename_list_1 = html_guangdong().get_filename_list_1()
        for i in filename_list_1:
            # print i[4]
            filename_path = r'D:\All_files\GuangDong\savehtml' + '\\' + i[4]
    ...
    def get_filename_path_list_2(self):
        ...
        filename_list_2 = html_guangdong().get_filename_list_2()
        for i in filename_list_2:
            # print i[4]
            filename_path = r'D:\All_files\GuangDong\savehtml' + '\\' + i[4]
            filename_path_list_2.append(filename_path)
            # print filename_path
        return filename_path_list_2
    ...
    def get_detail_1(self):
        ...
        filename_path_list_1 = html_guangdong().get_filename_path_list_1()[0]
        fbrq_list_1 = html_guangdong().get_filename_path_list_1()[1]
        for i in range(len(filename_path_list_1)):
            ...
            # print soup
            try:
                tbody = soup.find_all(id='BodyLabel')
                # print tbody
                all_tr = tbody[0].find_all('tr')
                # print all_tr
                tr_title = all_tr[0]
                fields = self.field
                match_fields = html_guangdong().get_field_info(tr_title,fields)
                ...
                if not match_fields:
                    ...
                    print 'match_fields = ', match_fields
                    changdu = 4
                for ii in range(changdu,len(all_tr)):
